...
public sealed class ShowtimeTypeController(IShowtimeTypeService service, IManagerCinemaScopeService scopes, IActivityLogService logs) : ControllerBase
{
 ...
 private async Task Log(string a,int id,string d,CancellationToken ct){if(!this.TryAuditActorId(out var adminId))return;await logs.RecordAsync(adminId,a,"ShowtimeType",id,d,this.AuditIpAddress(),ct);}
