...
public sealed record ActivityLogItem(int LogId, DateTimeOffset Timestamp, ActivityLogActor Actor,
    string ActionType, string Module, string Description, string IpAddress,
    int? TargetUserId, string? TargetTable, int? TargetId);
public sealed record ActivityLogDetail(int LogId, DateTimeOffset CreatedAt, int ActorId,
